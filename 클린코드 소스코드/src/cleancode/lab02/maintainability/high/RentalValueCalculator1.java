package cleancode.lab02.maintainability.high;
...
class Room {
	...
	public void printRentalStatus() {
		...
		for (int i = 0; i < residents.size(); i++)
			System.out.println("거주자 이름 : " + ((Resident) residents.get(i)).getName() + "," + " 임대비 : "
					+ ((Resident) residents.get(i)).calculateRentalValue());
...
public class RentalValueCalculator1 {
	...
	public static void main(String[] args) {
		...
		Room doolLiRoom1 = new Room("100");
		doolLiRoom1.addResident(new Adult("둘리"));
		doolLiRoom1.addResident(new Adult("또치"));
		doolLiRoom1.addResident(new Adult("고길동"));
		doolLiRoom1.addResident(new Children("희동이"));
		houses[0].addRoom(doolLiRoom1);
		
		Room doolLiRoom2 = new Room("101");
		doolLiRoom2.addResident(new Adult("마이콜"));
		houses[0].addRoom(doolLiRoom2);
		
		Room doolLiRoom3 = new Room("102");
		doolLiRoom3.addResident(new Adult("도우너"));		
		houses[0].addRoom(doolLiRoom3);
